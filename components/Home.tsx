  ...
  const renderCommentText = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@')) {
        ...
        if (exists) {
          return <span key={i} className="text-red-600 font-black bg-red-50 dark:bg-red-900/30 px-1.5 py-0.5 rounded-lg border border-red-100 dark:border-red-800">{part}</span>;
        }
      }
      return part;
    });
          ...
          <div className="space-y-4">
            {myTasks.length > 0 ? myTasks.map(task => (
              <button
                ...
                className="w-full text-left bg-white dark:bg-slate-800 p-8 rounded-[32px] border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl hover:border-red-600/50 hover:scale-[1.01] transition-all group"
              >
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-700">
                  ...
                  <ArrowRight size={18} className="text-slate-200 dark:text-slate-600 group-hover:text-red-600 group-hover:translate-x-2 transition-all" />
                </div>
              </button>
            )) : (
              <div className="bg-white dark:bg-slate-800 p-16 rounded-[40px] border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-lg font-black text-slate-300 dark:text-slate-500 uppercase tracking-widest">All Clear</p>
        ...
        <section className="space-y-8">
          ...
          <div className="space-y-4">
            {myNotifications.length > 0 ? myNotifications.map(n => {
              ...
              >
                <div className={`w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center font-black ${isBroadcast ? 'bg-slate-900 text-white' : 'bg-red-50 dark:bg-red-900/30 text-red-600'}`}>
                  ...
                  <div className="flex gap-4">
                    ...
                    {!n.read && (
                      <button 
                        ...
                        className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 dark:hover:text-slate-300"
                      >
                        Acknowledge
                      </button>
                    )}
              ...
              <div className="bg-white dark:bg-slate-800 p-16 rounded-[40px] border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-lg font-black text-slate-300 dark:text-slate-500 uppercase tracking-widest">No Alerts</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[40px] w-full max-w-2xl h-[80vh] flex flex-col shadow-2xl overflow-hidden border-t-8 border-red-600">
                ...
                <div className="flex items-center gap-2">
                  {isCoach && (
                    <button 
                      ...
                      className="p-3 bg-red-50 dark:bg-red-900/30 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-xl transition-all"
                      title="Delete announcement"
                    >
                      <Trash2 size={20} />
                    </button>
                  )}
                  <button onClick={() => setSelectedAnnouncement(null)} className="p-3 bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-red-600 rounded-xl transition-all">
                    <X size={24} />
                  </button>
                </div>
             </div>
             ...
             <div className="p-8 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 relative">
                ...
                {mentionFilter !== null && filteredMentionUsers.length > 0 && (
                  <div className="absolute bottom-[80px] left-8 right-8 z-20 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                    {filteredMentionUsers.map(u => (
                      <button 
                        key={u.id}
                        onClick={() => insertMention(u.username)}
                        className="w-full p-4 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors border-b border-slate-100 dark:border-slate-700 last:border-0"
                      >
                ...
                <div className="flex gap-3">
                   <div className="relative flex-1">
                      <input 
                         ...
                         className="w-full text-sm p-5 bg-white dark:bg-slate-700 border-2 border-slate-100 dark:border-slate-600 rounded-[24px] outline-none focus:border-red-600 transition-all pr-12 font-medium dark:text-white"
                      />
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-800 rounded-[40px] w-full max-w-xl p-12 shadow-2xl border-t-8 border-red-600">
            <div className="flex justify-between items-start mb-8">
              ...
              <button onClick={() => setIsBroadcasting(false)} className="p-3 bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-red-600 rounded-xl transition-all">
                <X size={24} />
              </button>
            </div>

            <div className="space-y-6">
              <div className="flex bg-slate-100 dark:bg-slate-700 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-600">
                {canBroadcastGlobal && (
                  <button 
                    onClick={() => setBroadcastScope('Global')}
                    className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${broadcastScope === 'Global' ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                  >
                    Global
                  </button>
                )}
                {canBroadcastDept && (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${broadcastScope === 'Department' ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                  >
                    Department
                  </button>
                )}
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full h-40 p-8 bg-slate-50 dark:bg-slate-700 border-2 border-slate-100 dark:border-slate-600 rounded-[32px] outline-none focus:border-red-600 transition-all font-bold text-slate-700 dark:text-slate-300 resize-none"
                />
              </div>
