...
const Home: React.FC<HomeProps> = ({ state, onTaskClick, onClearNotification, onAddAnnouncement, onUpdateAnnouncement, onDeleteAnnouncement, onNotify }) => {
  ...
  const { fmtDateOnly, fmtDateShort, fmtTime } = useTimeFormatters();
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center border-t border-white/5 pt-3">
                     ...
                     <span className="text-[8px] font-bold text-slate-500 uppercase">{fmtDateOnly(ann.timestamp)}</span>
                  </div>
                </div>
              );
            })}
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">
                        {fmtDateShort(evt.startDate)}
                        {evt.endDate && ` – ${fmtDateShort(evt.endDate)}`}
                      </p>
                    </div>
                    {!isDone && (
