...
const STORAGE_KEY = 'piobytes-active-board';
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ state, onUpdateTask, onDeleteTask, onAddTask, onAddProject, onUpdateProject, onArchiveProject, onNotify }) => {
  ...
  React.useEffect(() => {
    if (!accessibleProjects.find(p => p.id === activeProjectId)) {
      setActiveProjectIdRaw(firstAccessibleProject);
      try {
    ...
    <div className="h-full flex flex-col gap-4 md:gap-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-xl border border-slate-200 dark:border-slate-700 shadow-inner">
            <button 
                onClick={() => setView('board')} 
                className={`px-3 md:px-6 py-2 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${view === 'board' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
                Board
            </button>
            <button 
          ...
          {view === 'board' && (
            <>
              <select 
                ...
                className="flex-1 min-w-0 sm:flex-none sm:min-w-[160px] md:min-w-[200px] px-3 md:px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 font-bold text-sm text-slate-800 dark:text-slate-200 shadow-sm outline-none focus:ring-2 focus:ring-red-600/20"
              >
              ...
              <button 
                  ...
                  className="p-2 md:p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors shadow-sm"
              ...
              <select 
                ...
                className="hidden sm:block px-3 md:px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-slate-600 dark:text-slate-300 shadow-sm outline-none focus:ring-2 focus:ring-red-600/20"
              >
            ...
            {STATUSES.map(status => (
              ...
              >
                <div className="p-3 md:p-4 2xl:p-6 flex items-center justify-between bg-white/50 dark:bg-slate-800/50 rounded-t-xl md:rounded-t-2xl 2xl:rounded-t-[32px] border-b border-slate-100 dark:border-slate-700">
                  ...
                  <span className="bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] md:text-[10px] font-black px-2 py-0.5 md:py-1 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="grid grid-cols-1 gap-3 md:gap-4">
              {helpWantedTasks.map(task => (
                <div 
                  key={task.id} 
                  onClick={() => setSelectedTask(task)}
                  className="bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl md:rounded-[32px] p-4 md:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 hover:border-red-600/30 transition-all cursor-pointer"
                >
                  <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <button 
                    ...
                    className="w-full sm:w-auto px-4 md:px-6 py-2 md:py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-red-600 font-black text-[9px] md:text-[10px] rounded-xl md:rounded-2xl hover:bg-red-600 hover:text-white transition-all uppercase tracking-widest shadow-sm"
                  >
                    Resolve SOS
                  </button>
                </div>
              ...
              {helpWantedTasks.length === 0 && (
                <div className="py-16 md:py-24 text-center">
                  <CheckCircle size={48} className="text-slate-100 dark:text-slate-700 mx-auto mb-4 md:mb-6" />
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[110] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-[40px] w-full max-w-xl p-6 md:p-12 shadow-2xl border-t-8 border-red-600">
            <div className="flex justify-between items-start mb-6 md:mb-10">
              ...
              <button onClick={() => setShowNewProjectModal(false)} className="p-2 md:p-3 bg-slate-50 dark:bg-slate-900 text-slate-400 hover:text-red-600 rounded-xl transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-4 md:space-y-8">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full p-4 md:p-6 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-xl md:rounded-[28px] outline-none focus:border-red-600 transition-all font-black text-base md:text-lg uppercase tracking-tight text-slate-900 dark:text-white placeholder:text-slate-400"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full h-24 md:h-32 p-4 md:p-6 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-xl md:rounded-[28px] outline-none focus:border-red-600 transition-all font-bold text-slate-700 dark:text-slate-300 resize-none placeholder:text-slate-400"
                />
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl md:rounded-[40px] w-full max-w-xl p-6 md:p-12 shadow-2xl border-t-8 border-black dark:border-slate-600">
            ...
            <div className="space-y-4 md:space-y-6">
              <textarea 
                ...
                className="w-full h-24 md:h-32 p-4 md:p-6 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-xl md:rounded-[32px] outline-none focus:border-black dark:focus:border-slate-500 transition-all font-medium text-slate-700 dark:text-slate-300 resize-none placeholder:text-slate-400"
              />
              <div className="flex gap-3 md:gap-4">
                <button onClick={() => setBlockingTask(null)} className="flex-1 py-3 md:py-5 text-slate-400 font-black hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl md:rounded-[24px] uppercase tracking-widest text-[9px] md:text-[10px]">Cancel</button>
                <button 
...
const TaskCard: React.FC<{ 
    ...
    return (
        <div 
            ...
            className={`group bg-white dark:bg-slate-800 p-2.5 md:p-3 rounded-lg md:rounded-xl border shadow-sm hover:shadow-md hover:scale-[1.01] transition-all cursor-grab active:cursor-grabbing relative overflow-hidden ${
                task.helpRequested ? 'border-red-600 shadow-red-600/5' : 'border-slate-100 dark:border-slate-700 hover:border-red-600/30'
            }`}
        >
            {task.helpRequested && (
            ...
            <div className="flex justify-between items-start mb-1.5">
                <div className="flex gap-1">
                    ...
                    <span className="text-[6px] md:text-[7px] font-black px-1 md:px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded border border-slate-200 dark:border-slate-700">
                        {task.effort}pt
                    </span>
                </div>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <div className="flex -space-x-1">
                        ...
                        {task.assignees.length > 2 && (
                            <div className="w-4 h-4 md:w-5 md:h-5 rounded bg-slate-200 dark:bg-slate-600 text-slate-500 dark:text-slate-400 border border-white dark:border-slate-800 flex items-center justify-center text-[6px] md:text-[7px] font-black">
                                +{task.assignees.length - 2}
                            </div>
                        )}
                    </div>
                ...
                <button 
                    ...
                    className={`p-1 rounded transition-all ${
                        task.helpRequested ? 'bg-red-600 text-white shadow' : 'bg-slate-50 dark:bg-slate-700 text-slate-300 dark:text-slate-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950'
                    }`}
                    title="Request SOS"
                >
                    <LifeBuoy size={10} />
