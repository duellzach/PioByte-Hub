...
- **Calendar Page** (`/calendar`): Month-view calendar and list view backed by `calendar_events` DB table. Coaches/Captains can add, edit, and delete events via a modal form (title, type, start/end date, time, location, notes). Type color coding: practice=blue, competition=red, meeting=purple, deadline=amber. Upcoming events sidebar. Event detail sidebar on date click with edit/delete controls. Seeded with existing 7 events on first run.
