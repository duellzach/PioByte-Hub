...
- **FRC Scout Module**:
    ...
    - **Analysis & Display**: Robot dashboard with search, detailed views including cross-event match data, Pit Display with two sub-tabs (Live: 2-column 4K-optimized grid with Nexus upcoming matches + event schedule; Rankings: full TBA event standings with team 10991 highlighted + scouted robot leaderboard). **Pit Map** ("Map" tab per event): interactive SVG pit layout from FRC Nexus; search by team number OR team name (resolved from scouted pit data), auto-scrolls to pit, highlights searched team in orange; all error states have a "Try Again" button.
    ...
    - **Schedule Grid Enhancements (Task #41)**: "Scouts on duty" count row (first tbody row) shows per-match Scout-on-Stands count excluding off-marked members. Scouted team numbers from `teamClaims` appear in Scout-Stands cells. Off/break marking: coaches click any filled cell → modal with "Mark Off / Break" / "✓ Remove Off / Break Mark" toggle + "Edit Assignment Range". Off cells show diagonal stripe pattern. New `match_exceptions` DB table; REST API at `GET/POST/DELETE /api/events/:id/match-exceptions`; `api.matchExceptions.*` client methods.
...
- **Coach Onboarding Tutorial**: First-time dismissible 8-step tutorial modal (`components/CoachTutorial.tsx`) shown automatically to Coach-role users on first login. Progress tracked via `localStorage` key `piobyte_coach_tutorial_v1`. Steps cover: Welcome, War Room, Time Tracking, Team Management, Announcements, Scout, Safety Certs, Resources & Calendar. Includes step-dot navigation, Back/Got It/Skip All controls, and direct "Go there →" navigation links per step. Dismissed permanently once user completes or skips.
- **Seed Data Cleanup**: `server/storage.ts` default seed now creates 8 role-representative placeholder accounts (coach_mentor, captain10991, mech_lead, sw_lead, elec_lead, biz_lead, safety_trainer, member1) with password `changeme` and the project `2026 Competition Robot`. Login screen placeholder and seed-success alert updated to match.
- **Welcome Documentation**: `WELCOME_GUIDE.md` — comprehensive Google-Doc-ready guide covering all features by role, with 14 labeled `[SCREENSHOT: ...]` call-outs for manual capture.
